...
def get_parser():
    ...
    # optim
    parser.add_argument(
        '--base-lr', type=float, default=0.2, help='initial learning rate')
    parser.add_argument(
        '--step',
        type=int,
        default=[20, 40, 60],
...
class Processor():
    ...
    def load_optimizer(self):
        ...
        # 渐进式预热调度器（Gradual Warmup Scheduler）是一种在深度学习训练过程中用于调整学习率的策略。
        # 它特别适用于大型模型或者当使用较大的学习率可能导致训练不稳定的情况。
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    # 输出结果存到pred.npy文件数组里
                    out_put_cpu = output.cpu()
                    confidence = F.softmax(out_put_cpu, dim=1)
                    confidences_list = confidence.numpy()  # Convert to numpy array
                    score_frag.append(confidences_list)
            ...
            score_dict = dict(
                zip(self.data_loader[ln].dataset.label, score))
            ...
            if save_score:
                np.save('{}/pred.npy'.format(self.arg.work_dir), score)
